...
def these_are_colliding(obj_1, obj_2):
    hits = pygame.sprite.spritecollide(obj_1, obj_2, False)
    if len(hits) > 0:
        return(True)
    else:
        return False
...
def draw_grid(surface, width, height, grid_size, offset_x=0, offset_y=0, font=load_font(None, 16), color = (125, 125, 125)):
